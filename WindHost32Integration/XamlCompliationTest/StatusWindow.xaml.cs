...
namespace TESA.Runner.ProjectService
{
    ...
    public partial class StatusWindow : Window
    {
        ...
        public StatusWindow(LauncherStatus initialStatus)
        {
            ...
            {
                _logger = serviceProvider.Services.GetRequiredService<ILogger<StatusWindow>>();
                _statusService = serviceProvider.Services.GetRequiredService<StatusService>();
            }
            else
            {
                ...
                selectedItem.Tag is LaunchedProcessInfo processInfo)
            {
                ...
                if (result == MessageBoxResult.Yes)
                {
                    Task.Run(async () =>
                    {
                        try
                        {
                            var success = true;/*await _processLauncherService.StopProcessAsync(processInfo.ProcessId)*/;
